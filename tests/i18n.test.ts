...
test('a short key is not translated inside a longer word', () => {
  assert.equal(translate('Адрес', 'en'), 'Address');
  assert.equal(translate('Адресаты · 3', 'en'), 'Recipients · 3');
  assert.equal(translate('Источник контакта', 'en'), 'Contact source');
});
