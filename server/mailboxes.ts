...
import {detectProvider,oauthConfig,oauthReady,exchangeCode,sendMessage,verifySmtp,type Provider} from './mailproviders';
...
const secretKey=(email:string)=>`mailbox:${email.toLowerCase()}`;
export const redirectUri=()=>`${publicAddress(process.env)}/oauth/mailbox/callback`;

type Pending={email:string;provider:Provider;expires:number};
...
async function markConnection(email:string,connection:'oauth'|'smtp',provider:Provider){
 return change(s=>{const {domain,mailbox}=ensure(s,email);
...
export const mailboxOperations={
 ...
 detect:async(input:unknown)=>{const {email}=mailboxSchemas.email.parse(input);
  const detection=await detectProvider(email);
  const route=detection.provider==='smtp'?'smtp':oauthReady(detection.provider)?'oauth':'oauth-unconfigured';
  return {...detection,route,
   oauthConfigured:detection.provider!=='smtp'&&oauthReady(detection.provider),
   redirectUri:redirectUri()};},

 /** Builds the provider consent URL. The mailbox stays unconnected until the callback succeeds. */
 startOauth:async(input:unknown)=>{const {email,provider}=mailboxSchemas.start.parse(input);
  const detection=await detectProvider(email);
  ...
  const config=oauthConfig(chosen);
  if(!config)throw Error(`OAuth для ${chosen} не настроен на сервере: задайте client id и secret.`);
  const state=randomBytes(24).toString('base64url');
  pending.set(state,{email:email.toLowerCase(),provider:chosen,expires:Date.now()+900000});
 ...
 completeOauth:async(code:string,state:string)=>{
  ...
  const tokens=await exchangeCode(entry.provider,code,redirectUri());
  await setAuth(secretKey(entry.email),{kind:'oauth',provider:entry.provider,email:entry.email,
   refreshToken:tokens.refreshToken,scope:tokens.scope,at:new Date().toISOString()});
  await markConnection(entry.email,'oauth',entry.provider);
  return {email:entry.email,provider:entry.provider};},

 connectSmtp:async(input:unknown)=>{const body=mailboxSchemas.smtp.parse(input);
  ...
  await setAuth(secretKey(secret.email),{...secret,at:new Date().toISOString()});
  return markConnection(secret.email,'smtp','smtp');},

 /** The only proof a mailbox works: one real message through the connected account. */
 testSend:async(input:unknown)=>{const {email,to}=mailboxSchemas.test.parse(input);
  const before=await read();
  findMailbox(before,email);
  const secret=await getAuth<any>(secretKey(email));
  ...
  try{const sent=await sendMessage(secret,target,subject,text);result={status:'ok',detail:`Доставлено через ${sent.via}`};}
  catch(e:any){result={status:'failed',detail:String(e.message).slice(0,300)};}
  return change(s=>{const {domain,mailbox}=findMailbox(s,email);
 ...
 disconnect:async(input:unknown)=>{const {email}=mailboxSchemas.email.parse(input);
  await setAuth(secretKey(email),null);
  return change(s=>{const {domain,mailbox}=findMailbox(s,email);
 ...
 status:async()=>{const s=await read();
