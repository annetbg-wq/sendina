...
export async function read():Promise<State>{return normalize(pool?(await pool.query('SELECT data FROM workspace_state WHERE id=$1',['local'])).rows[0].data:JSON.parse(await readFile(join(dir,'state.json'),'utf8')));}
export function change<T>(fn:(s:State)=>T|Promise<T>):Promise<T>{
 const job=chain.then(async()=>{
  if(pool){const c=await pool.connect();try{await c.query('BEGIN');const s=normalize((await c.query('SELECT data FROM workspace_state WHERE id=$1 FOR UPDATE',['local'])).rows[0].data);const result=await fn(s);await c.query('UPDATE workspace_state SET data=$1 WHERE id=$2',[JSON.stringify(s),'local']);await c.query('COMMIT');return result;}catch(e){await c.query('ROLLBACK');throw e;}finally{c.release();}}
